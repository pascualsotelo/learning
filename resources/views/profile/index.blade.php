    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('profile.update') }}" novalidate>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="email" type="email" readonly class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email"
                                    value="{{ old('email') ?: $user->email }}" required autofocus />                                @if($errors->has('email'))
            ...
            @if(!$user->teacher)
                <div class="card card-whithout">
                    <div class="card-header">
                        {{__("Conviertete en Instructor")}}
                    </div>
                    <div class="card-body">
                        <form action="{{route('solicitude.teacher')}}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-outline-primary btn-block">
                                <i class="fa fa-graduation-cap"></i> {{__("Empieza a Enseñar")}}
                            </button>
                        </form>
                    </div>
                </div>
                ...
                <div class="card card-whithout">
                    ...
                    <div class="card-body">
                        <a href="{{route('teacher.courses')}}" class="btn btn-secondary btn-block">
                            <i class="fa fa-leanpub"></i> {{__("Administrar")}}
                        </a>
                    </div>
                </div>
                <div class="card card-whithout">
                    ...
                    <div class="card-body">
                        <table class="table table-hover table-striped table-bordered nowrap" id="students-table">
                            <thead>
                                <tr>
                                    <th>{{__("ID")}}</th>
                                    <th>{{__("Nombre")}}</th>
